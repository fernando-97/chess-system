...
public class ChessMatch {
    ...
    public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
        ...
        ChessPiece capturePiece = (ChessPiece) makeMove(source, target);
    ...
    private void validateSourcePosition(Position position) {
        if (!board.theresIsAPiece(position)) {
            throw new ChessException("Não há peça nessa posição do tabuleiro.");
        }
        if (currentPlayer != ((ChessPiece) board.piece(position)).getColor()) {
            throw new ChessException("A peça escolhida não é sua.");
        }
        if (!board.piece(position).isThereAnyPossibleMove()) {
            throw new ChessException("Não opções de movimento para esta peça.");
        }
    }

    private void validateTargetPosition(Position source, Position target) {
        if (!board.piece(source).possibleMove(target)) {
            throw new ChessException("A peça escolhida não pode mover para a posição escolhida");
        }
    }
    ...
    public ChessPiece replacePromotedPiece (String type) {
        ...
        if (!type.equals("B") && !type.equals("N") && !type.equals("Q") && !type.equals("k")) {
            return promoted;
        }
    ...
    private void initialSetup() {
        ...
        placeNewPiece('e', 2, new King(board, Color.WHITE, this));
        placeNewPiece('f', 2, new Pawn(board, Color.BLACK, this));

        placeNewPiece('e', 7, new King(board, Color.BLACK, this));
